...
import { SpotifyPlaybackSDK, } from './spotifyPlaybackSDK'
...
export async function switchDevice() {
    ...
    }).catch(e => console.log(e, { depth: null }));
}
    ...
    if (!player) {
        ...
        player.on("player_state_changed", (data: any) => {
            if (!data.paused) {
                // console.log(data);
                // return
                client.API.game.startMusicActivity('kook-ongaku-play', data.track_window.current_track.name);
...
export async function streamSpotifyToChannel(channelId: string): Promise<void> {
    ...
    let browserAudio = await player.getAudio();
    browserAudio.on('data', (chunk) => {
        stream.push(chunk);
    });
    voice = new koice(await client.config.getOne('kasumi::config.token'));
    await voice.connectWebSocket(channelId);
