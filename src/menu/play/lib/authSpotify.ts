...
try {
    auth = JSON.parse(fs.readFileSync(upath.join(__dirname, "..", "..", "..", "config", "data.json"), { encoding: "utf-8", flag: 'r' }));
} catch (e) {
    auth = {};
}
...
export async function loginSpotify() {
    ...
    fs.writeFileSync(upath.join(__dirname, "..", "..", "..", "config", "data.json"), JSON.stringify({
